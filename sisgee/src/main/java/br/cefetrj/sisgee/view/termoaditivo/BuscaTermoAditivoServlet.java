...
public class BuscaTermoAditivoServlet extends HttpServlet {
	...
	/**
	 * 
	 * @param request  um objeto HttpServletRequest que contém a solicitação feita
	 *                 pelo cliente do servlet.
	 ...
	 * @throws IOException      se um erro de entrada ou saída for detectado quando
	 *                          o servlet manipula o pedido
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Integer id = null;
		boolean existeTermoAtivo = false;
		...
		if (Integer.parseInt(idAluno) != -1) {
			...
			if (termoEstagios != null) {				
				if (!aluno.getTermoEstagios().isEmpty()) {
					List<TermoAditivo> termoAditivos = aluno.getTermoEstagios().get(aluno.getTermoEstagios().size() - 1)
							.getTermosAditivos();
					Collections.sort(termoAditivos);
					request.setAttribute("termosAditivos", termoAditivos);
				}

				// Testando Se existe um termo de estágio ativo, caso haja,
				// seta a variável para mostrar o botão de encerramento de termo.
				for (TermoEstagio termoEstagio : termoEstagios) {
					if (Constantes.ATIVO.equalsIgnoreCase(termoEstagio.getEstado())) {
						existeTermoAtivo = true;						
					}
				}
			}
			request.setAttribute("existeTermoAtivo", existeTermoAtivo);
